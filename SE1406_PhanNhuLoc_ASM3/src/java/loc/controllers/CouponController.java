...
public class CouponController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (valid == true) {
                discountAmount = dao.getDiscountAmount(code);
                if (discountAmount < 1) {
                    valid = false;
                }
            }
            ...
            if (valid == true) {
                total = total - (total * (Double.valueOf(discountAmount) / 100));
                session.setAttribute("TOTAL", total);
                session.setAttribute("DISCOUNT_CODE", code);
            } else {
                session.setAttribute("TOTAL", total);
                session.setAttribute("DISCOUNT_CODE", null);
                request.setAttribute("COUPON_ERROR", "INVALID CODE");
            }
            url = SUCCESS;
        } catch (Exception e) {
            LOGGER.error("Error at CouponController: " + e.toString());
        } finally {
